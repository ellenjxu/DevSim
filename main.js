...
app.post("/api/reset", async (req, res) => {
	const mujocoDemo = require("./src/examples/mujocoUtils.js");
	mujocoDemo.resetSimulation();
...
function getSimulationData() {
	// returns state (e.g. time step, positoin of mass)
	let time = MuJoCoDemo.mjData.time;
	let mass = MuJoCoDemo.mjData.qpos[0];
	return {time, mass};
}

app.get("/api/getState", (req, res) => {
	let data = getSimulationData();
	res.json(data);
});
...
import {DragStateManager} from "./public/examples/utils/DragStateManager.js";
import {
	setupGUI,
	...
	toMujocoPos,
	standardNormal,
} from "./public/examples/mujocoUtils.js";
...
var initialScene = "pendulum.xml";
mujoco.FS.mkdir("/working");
mujoco.FS.mount(mujoco.MEMFS, {root: "."}, "/working");
var files = await fs.promises.readdir("./examples/scenes");
console.log(files);
mujoco.FS.writeFile(
